...
def convert_to_skill(
    mcp_config: MCPConfig, skill_config: Optional[SkillConfig] = None
) -> SkillInfo:
    """
    ...
    try:
        ...
        # Determine output directory (project-local .claude/skills/{name})
        default_output = Path.cwd() / ".claude" / "skills" / mcp_config.name
        ...
        for line in result.stdout.split("\n"):
            if "Tools available:" in line:
                ...
                tools = [{"name": f"tool_{i}", "description": f"Tool {i}"} for i in range(count)]
                break
...
def validate_skill(skill_path: str) -> Dict[str, Any]:
    ...
    required_files = ["executor.py", "mcp-config.json", "SKILL.md"]
    ...
    try:
        ...
        if result.returncode != 0:
            return {"valid": False, "error": result.stderr, "files": required_files}
...
def test_skill(
    ...
    try:
        ...
        if result.returncode != 0:
            return {"success": False, "error": result.stderr, "mode": mode}
        ...
        return {"success": True, "mode": mode, "output": output}
